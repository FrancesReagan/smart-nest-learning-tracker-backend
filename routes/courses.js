...
router.get("/:id", async (req, res) => {
  try {
    ...
    if (course.user._id.toString() !== req.user._id.toString()) {
      return res
        .status(403)
        .json({ message: "Access denied -- not your course" });
    }
...
router.put("/:id", async (req, res) => {
  try {
    ...
    if (courseToUpdate.user.toString() !== req.user._id.toString()) {
      return res
        .status(403)
        .json({ message: "Access is denied as this is not your course" });
    }
...
router.delete("/:id", async (req, res) => {
  try {
    ...
    if (!courseToDelete) {
      return res
        .status(404)
        .json({ message: "Course not found...can not delete unknown course" });
    }
    ...
    if (courseToDelete.user.toString() !== req.user._id.toString()) {
      return res
        ...
        .json({
          message:
            "Access denied -- you can not delete course that is not yours.",
        });
    }
