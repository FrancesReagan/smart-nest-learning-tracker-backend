...
router.post("/courses/:courseId/sessions", async (req, res) => {
  try {
    ...
    if (course.user.toString() !== req.user._id.toString()) {
      return res
        .status(403)
        .json({ message: "Access Denied---you can not access this..." });
    }
    //  to create a session//
  ...
  try {
    ...
    if (session.course.user.toString() !== req.user._id.toString()) {
      return res
        .status(403)
        .json({
          message: "Access is denied-- you don't own this session's course",
        });
    }
    res.json(session);
  } catch (error) {
...
router.put("/courses/:courseId/sessions/:sessionId", async (req, res) => {
  try {
    ...
    if (session.course.user.toString() !== req.user._id.toString()) {
      return res
        .status(403)
        .json({ message: "Sorry you don't have the rights to access..." });
    }
...
router.delete("/courses/:courseId/sessions/:sessionId", async (req, res) => {
  try {
    ...
    if (!session) {
      return res
        .status(404)
        .json({ messaege: "Session not found in the course" });
    }
